...
// 							bbVarAdded = true;
// 							menu.AddItem(new GUIContent(string.Format("Variables[变量]/Get Blackboard Variable/Get {0}", finalName), null, "Variable from Blackboard"), false, ()=>
// 							{
...
// 					}
				...
				if (!bbVarAdded){
					//menu.AddDisabledItem(new GUIContent("Variables[变量]/Get Blackboard Variable") );
				}
			}

			if (clickedPort is ValueOutput){

// 				menu.AddItem(new GUIContent(string.Format("Variables[变量]/Set Blackboard Variable/Set Of Type ({0})", clickedPort.type.FriendlyName() )), false, ()=>
// 				{
// 					var genericType = typeof(SetVariable<>).MakeGenericType(new System.Type[]{clickedPort.type});
// 					SelectedFlowNode(genericType);
// 				});
// 
// 				menu.AddItem(new GUIContent(string.Format("Variables[变量]/Set Blackboard Variable/Set Other Of Type ({0})", clickedPort.type.FriendlyName() )), false, ()=>
// 				{
...
// 							bbVarAdded = true;
// 							menu.AddItem(new GUIContent(string.Format("Variables[变量]/Set Blackboard Variable/Set {0}", finalName), null, "Variable from Blackboard"), false, ()=>
// 							{
...
// 					}
				...
				if (!bbVarAdded){
					//menu.AddDisabledItem(new GUIContent("Variables[变量]/Set Blackboard Variable") );
				}
			}
		...
		protected override GenericMenu OnContextMenu(GenericMenu menu){
			...
			if (outputPorts.Values.Any(p => p is FlowOutput)){ //breakpoints only work with FlowOutputs
				menu.AddItem(new GUIContent("断点"), isBreakpoint, ()=>{ isBreakpoint = !isBreakpoint; });
			}
			...
			if (type.IsGenericType){
				...
				foreach(var _info in infos){
					var info = _info;
					menu.AddItem(new GUIContent("改变类型/" + info.name), false, ()=>
					{
